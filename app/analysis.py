...
def calculate_metrics(df, industry):
    ...
    results = pd.DataFrame(index = ['Beta Coefficients', 'P-Values', '(Correlations) Closing Price',
                                    '(Correlations) day-to-day Δ Closing Price', '(Correlations) 3-day Volatility'
                                   ],
                           columns = predictors)
    ...
    for var in predictors:
        # Correlations
        results.at['(Correlations) Closing Price', var]              = stats.pearsonr(df_filtered[var], df_filtered['close'])[0]
        results.at['(Correlations) day-to-day Δ Closing Price', var] = stats.pearsonr(df_filtered[var], df_filtered['pct_change_close'])[0]
        results.at['(Correlations) 3-day Volatility', var]           = stats.pearsonr(df_filtered[var], df_filtered['close_vol_3d'])[0]
    ...
    results.loc['Beta Coefficients', mlr_summary.index]    = mlr_summary['beta_mlr']
    results.loc['P-Values', mlr_summary.index] = mlr_summary['p_value_mlr']

    # Extract adjust R² of the full model
    adjusted_r_squared_mlr = round(mlr_model.rsquared_adj * 100, 2)

    results.rename(columns = {"publications": "Publications", "publications_1": "1 day lag (P)",
                              "publications_3": "3 day lag (P)", "publications_7": "7 day lag (P)",
                              "publications_14": "14 day lag (P)", "publications_30": "30 day lag (P)",
                              "citations": "Citations", "citations_1": "1 day lag (C)",
                              "citations_3": "3 day lag (C)", "citations_7": "7 day lag (C)",
                              "citations_14": "14 day lag (C)", "citations_30": "30 day lag (C)",
                             }, inplace = True)
